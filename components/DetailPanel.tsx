...
export const DetailPanel: React.FC<DetailPanelProps> = ({
  ...
  if (!currentImage) {
    return <div style={{ width }} className="h-full bg-slate-800 p-4 text-slate-500 shrink-0">{t.detail.noImage}</div>;
  }
  ...
  if (totalLabels === 0) {
    return (
      <div style={{ width }} className="h-full flex flex-col items-center justify-center p-8 bg-slate-800 border-l border-slate-700 text-slate-400 shrink-0">
        ...
        <h3 className="text-xl font-bold mb-2">{t.detail.noLabels}</h3>
        <p className="text-center text-sm mb-6">{t.detail.noLabelsDesc}</p>
        ...
        >
             <Plus size={20} />
             {isCreating ? t.detail.drawBtn : t.detail.startBtn}
        </button>
      </div>
    );
  }
    ...
    <div style={{ width }} className="h-full flex flex-col bg-slate-800 border-l border-slate-700 overflow-hidden shrink-0 shadow-xl transition-all duration-75">
        ...
        <div className="grid grid-cols-1 gap-2 bg-slate-800 p-3 rounded-lg border border-slate-700">
            ...
            <div className="flex items-center gap-3">
                 ...
                 <div className="flex-1 flex flex-col">
                     <span className="text-[10px] text-slate-500 uppercase font-bold">{t.app.grid.context}</span>
                     <input 
                        type="range" 
                        min="0" 
                        max="100" 
            ...
            {/* Magnification Slider */}
             <div className="flex items-center gap-3 mt-1">
                 ...
                 <div className="flex-1 flex flex-col">
                     <span className="text-[10px] text-slate-500 uppercase font-bold">{t.app.grid.magnification}</span>
                     <input 
                        type="range" 
                        min="1" 
                        max="10" 
      ...
      <div className="p-6 flex-1 flex flex-col gap-6 overflow-y-auto">
        ...
        <div className="space-y-4 p-4 bg-slate-900/30 rounded-lg border border-slate-700/50">
           <h3 className="text-xs font-bold text-slate-400 uppercase">{t.detail.coords}</h3>
